...
#if defined(ENABLE_DAMAGE_PRINT_IMPROVE)
	...
	*/
	...
	if (isCritical || isPenetrate)
	{
		...
		if (isCritical && isPenetrate)
		{
			...
			c_szPrefixEffectPath = "d:/ymir work/effect/affect/damagevalue/damage_mix.mse";
		}
		else if (isCritical)
		{
			pdwPrefixCRC = &gs_dwDamagePrefixCriticalCRC;
			c_szPrefixEffectPath = "d:/ymir work/effect/affect/damagevalue/damage_critical.mse";
		}
		else
		{
			pdwPrefixCRC = &gs_dwDamagePrefixPenetrationCRC;
			c_szPrefixEffectPath = "d:/ymir work/effect/affect/damagevalue/damage_penetration.mse";
		}
		...
		if (pdwPrefixCRC && *pdwPrefixCRC)
		{
			...
			const float fPrefixPadding = 18.0f;
			while (tempDamage > 0)
			{
				++digitCount;
				tempDamage /= 10;
...
void CInstanceBase::__ComboProcess()
{
	...
	if (0 != dwcurComboIndex)
	{
		...
		{
			if (!m_GraphicThingInstance.IsHandMode() && IsAffect(AFFECT_GEOMGYEONG))
			{
