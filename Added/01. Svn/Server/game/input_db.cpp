...
void CInputDB::SafeboxLoad(LPDESC d, const char* c_pData)
{
	...
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_STRING("You cannot open a Storeroom while another window is open."));
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
	...
	if (p->flag)
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Storeroom] Storeroom password has been changed."));
		d->GetCharacter()->SetSafeboxBuff();
	}
	else
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[Storeroom] You have entered the wrong password."));
	}
}
...
void CInputDB::AddMonarchMoney(LPDESC d, const char* data)
{
	...
	{
		if (number(1, 100) > 95)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("[#Unk]%s treasury currently has %u of money.", EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire)));
	}
}
...
void CInputDB::DecMonarchMoney(LPDESC d, const char* data)
{
	...
	if (ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s still has %d Yang available.", EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire)));
	}
}
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), LC_STRING("The emperor of %s has changed to %s.", EMPIRE_NAME(info->bEmpire), info->szName));
	SendNotice(notice);
}
